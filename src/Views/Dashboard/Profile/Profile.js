...
export const Profile = () => {
  ...
  useEffect(() => {
    const getGroups = async () => {
      try {
        const { groupsAsAdmin } = await groupsService.getGroups({ userID: id, isAdmin: true, isProfileInfo: false })
        const { groupsAsMember } = await groupsService.getGroups({ userID: id, isAdmin: false, isProfileInfo: false })
